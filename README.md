...
## v65.33 サンプリング写真ルート / 仮案件フォルダ
- `03 サンプリング / サンプリング写真` を写真保存ルートとして解決・確認
- 正式案件検索ルートは `03 サンプリング` 直下をそのまま使用
- 仮案件は `サンプリング写真 / 端末名_yymmdd_枝番` を自動作成
- 同名フォルダは再利用し、接続復帰後にも再確保


## v65.34 OneDrive写真2種アップロード基盤
- 仮案件フォルダ直下へ看板入り完成画像を送信
- 仮案件フォルダ内の `元画像` へoriginalを送信
- original / completedを独立したpending/uploaded状態で管理
- Graph upload後、返却itemIdを再取得して実在確認してからuploaded扱い
- 現在案件だけを直列送信し、案件切替後の誤送信を防止
- 送信失敗はpendingのまま保持し、写真追加・OneDrive接続復帰・オンライン復帰時に再判定
- 看板添付で読み込んだ写真もOneDrive送信確認までは元画像を端末保持
- v65.34では元画像の端末削除はまだ行わない


## v65.35 OneDrive送信状態表示 / 案件別ファイル採番
- 写真サムネイル/一覧の右上へ文字なしのOneDrive状態●を追加
  - 緑: original/completedとも送信・実在確認済み
  - 黄: 送信中またはOneDrive保存確認中
  - 赤: 未送信/再送待ち
- 既存の「保存済」表示を「端末保存済」へ変更し、小型化
- 端末保存成功時の既存「保存しました」トーストは維持
- 写真ファイル名の `_02` / `_03` 採番を案件(caseId)ごとに独立
- 看板修正後はcompletedを再送対象へ戻し、ファイル名変更時はoriginalも再送対象へ戻す
- IndexedDB schema/versionは変更なし


## v65.36 端末保存済み表示位置調整
- 「端末保存済」バッジのサイズ・文言は変更せず、左上から左下へ移動
- OneDrive状態●、選択チェック、保存処理、看板仕様には変更なし


## v65.37 OneDrive同期と更新基盤の正式化
- 案件作成時の端末名 / OneDriveフォルダ名を案件単位で固定し、端末名変更は次の新規案件から反映
- 案件切替をOneDrive写真同期の再判定トリガーへ追加
- 写真の黄色●は送信/確認中だけの一時表示とし、IndexedDBへuploading/verifyingを永続化しない
- 元画像 + 完成画像の両方をOneDrive実在確認後に緑●へ移行
- 両方確認後は端末内元画像を解放し、看板修正時はOneDrive元画像を一時取得して再利用
- 看板修正後は元画像を再送せず、完成画像だけを新しい版としてOneDriveへ保存
- 完成画像の枝番はOneDrive上の既存ファイルを基準に送信時決定し、同名上書きを避ける
- 設定へ Microsoft / OneDrive の再接続と「今すぐ再送」を追加
- 「端末保存済」表示を廃止し、共有成功時は「保存 / 共有が完了しました」と表示
- PWA更新を version.json + waiting Service Worker切替 + 更新モーダルへ変更
- PWA更新時にlocalStorage / IndexedDB / 写真データを削除しない
